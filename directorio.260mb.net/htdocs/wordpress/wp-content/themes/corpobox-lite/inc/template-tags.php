...
function corpobox_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'corpobox-lite' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-arrow-left"></i>', 'Previous post link', 'corpobox-lite' ) . '</span><br /> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-arrow-right"></i>', 'Next post link', 'corpobox-lite' ) . '</span><br /> %title' ); ?>

<?php elseif ( $wp_query->max_num_pages > 1 && ( is_search() ) ) : // navigation links for search pages or is_home() || is_archive() ||  etc ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'corpobox-lite' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'corpobox-lite' ) ); ?></div>
		<?php endif; ?>
...
function corpobox_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
		<div class="comment-body">
			<?php _e( 'Pingback:', 'corpobox-lite' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'corpobox-lite' ), '<span class="edit-link">', '</span>' ); ?>
		</div>
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'corpobox-lite' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author -->

				<div class="comment-metadata">
					<time datetime="<?php comment_time( 'c' ); ?>">
	<?php printf( _x( '%1$s &middot; %2$s', '1: date, 2: time', 'corpobox-lite' ), get_comment_date(), get_comment_time() ); ?>
					</time>
	<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">#<?php echo $comment->comment_ID; ?></a>
					<?php edit_comment_link( __( 'Edit', 'corpobox-lite' ), '<span class="edit-link">', '</span>' ); ?>
				</div><!-- .comment-metadata -->

				<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'corpobox-lite' ); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
...
function corpobox_posted_on() {
	...
	printf( __( '<span class="posted-on">%1$s</span><span class="byline"> <i>by</i> %2$s</span>', 'corpobox-lite' ),
...
function corpobox_posted_extra() {
	...
	$categories_list = get_the_category_list( __( ', ', 'corpobox-lite' ) );
	$tags_list = get_the_tag_list( '', __( ', ', 'corpobox-lite' ) );

	if ( $categories_list && corpobox_categorized_blog() ) {

	echo '<span class="cat-links">' . __( 'Category: ', 'corpobox-lite' ), $categories_list . '.</span>';

	}
	if ( $tags_list ) {

	echo '<span class="tags-links">' . __( 'Tag: ', 'corpobox-lite' ), $tags_list . '</span>';
...
function corpobox_txt_credits() {
	$text = sprintf( __( 'Powered by %s', 'corpobox-lite' ), '<a href="http://wordpress.org/">WordPress</a>' );
	$text .= '<span class="sep"> &middot; </span>';
	$text .= sprintf( __( 'Theme by %s', 'corpobox-lite' ), '<a href="http://www.dinevthemes.com/">DinevThemes</a>' );
...
if ( ! function_exists( 'corpobox_cart_link' ) ) {
	function corpobox_cart_link() {
		...
		<li class="<?php echo esc_attr( $class ); ?>">
			<a href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php _e( 'View Cart', 'corpobox-lite' ); ?>">
