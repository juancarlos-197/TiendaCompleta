...
class Corpobox_Icon_Widget extends WP_Widget {
	function __construct() {
		parent::__construct(
			...
			'Corpobox - ' . __( 'Icons', 'corpobox-lite' ),
			array(
				'classname' => 'corpobox_icon_widget', 
				'description' => __( 'Custom Text Widget with Icons', 'corpobox-lite' ),
				'width' => 250,
				'height' => 350
			)
		);
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'corpobox-lite' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<p><label><?php _e( 'Icon:', 'corpobox-lite' ); ?></label>
		<span class="custom-icon-container"><?php echo $icon_tag; ?></span>
		<a href="#" class="view-icons"><?php _e( 'View Icons', 'corpobox-lite' ); ?></a> | <a href="#" class="delete-icon"><?php _e( 'Remove', 'corpobox-lite' ); ?></a>
		...
		<p><label for="<?php echo $this->get_field_id( 'url' ); ?>"><?php _e( 'URL:', 'corpobox-lite' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_text' ); ?>"><?php _e( 'Button Text:', 'corpobox-lite' ); ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'button_color' ); ?>"><?php _e( 'Icon and Button Color:', 'corpobox-lite' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'button_color' ); ?>" id="<?php echo $this->get_field_id( 'button_color' ); ?>" class="widefat">
		<?php
		$options = array(
			'default' => __( 'Default', 'corpobox-lite' ),
			'blue' => __( 'Blue', 'corpobox-lite' ),
			'red' => __( 'Red', 'corpobox-lite' ),
			'green' => __( 'Green', 'corpobox-lite' ),
		);
		...
		<p><input id="<?php echo $this->get_field_id('filter'); ?>" name="<?php echo $this->get_field_name('filter'); ?>" type="checkbox" <?php checked( isset( $filter ) ? $filter : 0 ); ?> />&nbsp;<label for="<?php echo $this->get_field_id('filter'); ?>"><?php _e( 'Automatically add paragraphs', 'corpobox-lite' ); ?></label></p>
		<?php
	}
}